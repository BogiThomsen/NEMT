...
device_url = 'http://device-service:5400/v1'
...
def patch_users_id(userid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            user_response = requests.patch(user_url + '/users/' + userid, headers=headers, json=request.json["data"])
            return make_response(user_response.content, user_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
...
def get_users_user_id_devices(userid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            device_response = requests.get(device_url + '/users/' + userid + '/devices', headers=headers, json=request.json["data"])
            return make_response(device_response.content, device_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
...
def patch_users_user_id_devices_device_id(userid, deviceid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            device_response = requests.patch(device_url + '/users/' + userid + '/devices/' + deviceid, headers=headers, json=request.json["data"])
            return make_response(device_response.content, device_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
...
def post_users_user_id_devices_device_id_actions(userid, deviceid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            action_response = requests.post(action_url + '/users/' + userid + '/devices/' + deviceid + '/actions', headers=headers, json=request.json["data"])
...
def get_users_user_id_devices_device_id_actions(userid, deviceid):
    validation = validate_actions_request(request)
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            action_response = requests.get(action_url + '/users/' + userid + '/devices/' + deviceid + '/actions', headers=headers, json=request.json["data"])
            return make_response(action_response.content, action_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
...
def patch_users_user_id_devices_device_id_actions_action_id(userid, deviceid, actionid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            action_response = requests.patch(action_url + '/users/' + userid + '/devices/' + deviceid + '/actions/' + actionid, headers=headers, json=request.json["data"])
            return make_response(action_response.content, action_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
...
def post_users_user_id_devices_device_id_sensors(userid, deviceid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            sensor_response = requests.post(sensor_url + '/users/' + userid + '/devices/' + deviceid + '/sensors', headers=headers, json=request.json["data"])
...
def get_users_user_id_devices_device_id_sensors(userid, deviceid):
    validation = validate_sensors_request(request)
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            sensor_response = requests.get(sensor_url + '/users/' + userid + '/devices/' + deviceid + '/sensors', headers=headers, json=request.json["data"])
            return make_response(sensor_response.content, sensor_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
...
def patch_users_user_id_devices_device_id_sensors_sensor_id(userid, deviceid, sensorid):
    ...
    if validation == '':
        authorization = authorize(request.json['accessToken'], userid)
        if authorization == '':
            sensor_response = requests.patch(sensor_url + '/users/' + userid + '/devices/' + deviceid + '/sensors/' + sensorid, headers=headers, json=request.json["data"])
            return make_response(sensor_response.content, sensor_response.status_code)
        else:
            return make_response(json.dumps({"error": authorization, "body": request.json}), 401, headers)
    else:
